/**
 * Crisp Lucide-style SVG Icon library for Vanilla JavaScript
 * Generates inline SVG elements with proper accessibility attributes
 */
...
export function icon(name, size = 16, className = '') {
  ...
  const svgInner = icons[name] || icons.file;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="${className}">${svgInner}</svg>`;
}

export function getFileIcon(type, size = 16) {
  switch (type) {
    case 'pdf':
      return icon('fileText', size, 'text-red-500');
    case 'xlsx':
    case 'csv':
      return icon('table', size, 'text-emerald-500');
    case 'image':
      return icon('image', size, 'text-blue-500');
    case 'transcript':
      return icon('bookOpen', size, 'text-amber-500');
    case 'docx':
    case 'txt':
    default:
      return icon('file', size, 'text-slate-400');
  }
}
